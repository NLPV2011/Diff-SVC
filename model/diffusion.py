...
class GaussianDiffusionShallow(nn.Module):
    def __init__(self, configs):
        super().__init__()
        self.denoise_fn = Denoiser(configs)
        self.mel_bins = configs["preprocessing"]["mel"]["n_mel_channels"]

        betas = get_noise_schedule_list(
            configs["denoiser"]["noise_schedule_shallow"],
            configs["denoiser"]["timesteps"],
            configs["denoiser"]["max_beta"],
            configs["denoiser"]["s"],
        )
        ...
        self.K_step = int(configs["denoiser"]["K_step"])
        self.loss_type = configs["loss"]["noise_loss"]
        ...
        with open(
                os.path.join(configs["path"]["preprocessed_path"], "stats.json")
        ) as f:
            stats = json.load(f)
            self.register_buffer("spec_min", torch.FloatTensor(stats["spec_min"])[None, None, :configs["denoiser"]["keep_bins"]])
            self.register_buffer("spec_max", torch.FloatTensor(stats["spec_max"])[None, None, :configs["denoiser"]["keep_bins"]])
        self.aux_mel = None
